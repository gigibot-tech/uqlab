...
def label_noise_experiment(x_train, y_train, x_test, y_test, model, config):
    ...
    for noise in noises:
        ...
        model.fit(X_train_noisy, y_train_noisy, label_noise=noise)
...
def label_noise_experiment_torch(train_dataset, val_dataset, model, config, batch_size, workers, train_only=False):
    ...
    for noise in noises:
        use_accel = True
        train_dataloader = create_partial_shuffle_dataloader_torch(
            train_dataset, percentage=noise, batch_size=batch_size, workers=workers
        )
        val_dataloader = create_partial_shuffle_dataloader_torch(
            val_dataset, percentage=noise, batch_size=batch_size, workers=workers
        )
    ...
    return (
        1 * np.abs(aleatoric_correlation - 1)
        + config.term_weights[2] * np.abs(epistemic_correlation - 0),
        experiment_results,
    )
...
def create_partial_shuffle_dataloader_torch(dataset, percentage, batch_size, workers):
    indices = np.random.choice(
        np.arange(len(dataset.samples)),
        size=int(percentage * len(dataset.samples)),
        replace=False,
    )
