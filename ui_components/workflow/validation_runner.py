...
def run_validation_experiments(
    ...
    *,
    ...
    """Run preset validation sweeps in-process via ``run_validation_sweep_inprocess``."""
    del timeout  # in-process; no subprocess timeout

    _runners = _PROJECT_ROOT / "scripts" / "runners"
    if str(_runners) not in sys.path:
        sys.path.insert(0, str(_runners))

    import run_validation_experiments as rve

    return rve.run_validation_sweep_inprocess(
        sweep_type,
        mode,
        on_line=on_line,
    )
