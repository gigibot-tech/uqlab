...
**Date:** 2026-05-15  
**Merged From:** `uq_classification/` (standalone) → `uqlab-streamlit/uq_classification/` (production)

## Summary

Successfully merged the standalone `uq_classification` package into `uqlab-streamlit/uq_classification/` to create a unified package with enhanced visualization capabilities while preserving all existing production functionality.
...
2. **streamlit_viz_app.py** - Interactive Streamlit dashboard (renamed from `streamlit_app.py`)
   - **Note:** Renamed to avoid conflict with existing `uqlab-streamlit/streamlit_app.py` (API dashboard)
   - Features: experiment selection, metrics visualization, decision boundary explorer
   - Usage: `streamlit run uqlab-streamlit/uq_classification/streamlit_viz_app.py`
...
- **Issue:** Both packages had a `streamlit_app.py` file with different purposes
  - `uqlab-streamlit/streamlit_app.py`: API dashboard for backend integration
...
- **Decision:** Kept `uqlab-streamlit/uq_classification/attribution_signals.py` (production version)
...
```
uqlab-streamlit/uq_classification/
...
1. **Install visualization dependencies:**
   ```bash
   pip install -r uqlab-streamlit/uq_classification/requirements_viz.txt
   ```

2. **Test the merged package:**
   ```bash
   # Test imports
   python -c "from uqlab_cen.uq_classification import ExperimentTracker, plot_decision_boundary"
   
   # Run example
   python uqlab-streamlit/uq_classification/example_decision_boundary.py
   
   # Launch visualization dashboard
   streamlit run uqlab-streamlit/uq_classification/streamlit_viz_app.py
   ```
