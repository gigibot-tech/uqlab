...
1. **collect_uncertainty_signals** — score each eval sample (attribution, logits, optional MC dropout).
2. **score_uncertainty_signals** — AUROC per signal: does it flag noisy labels? under-trained classes?
"""
...
from uqlab.evaluation.result_writers import save_per_sample_csv
...
def score_uncertainty_signals(
    *,
    ...
    Also trains a simple N-way classifier on signals (macro-F1) and writes per-sample CSV.
    """
